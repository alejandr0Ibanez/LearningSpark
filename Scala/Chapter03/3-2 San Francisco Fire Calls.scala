...
print(fireTSDF.printSchema())

// COMMAND ----------

fireTSDF
  .select("CallType", "IncidentDate")
  .where(col("IncidentDate")==="2018" && col("CallType").isNotNull)
  .distinct()
  .show(10, false)
...
display(fireTSDF
  .filter($"City" === "San Francisco" && year($"IncidentDate")===lit("2018"))
  .groupBy($"Neighborhood")
  .agg(count("*").as("conteo_barrio"))
  .orderBy($"conteo_barrio")
)
...
display(fireTSDF
  .filter(year($"IncidentDate")===lit("2018"))
  .groupBy($"Neighborhood")
  .agg(avg($"ResponseDelayedinMins").alias("tiempo_res"))
  .orderBy(desc("tiempo_res"))
)
...
display(fireTSDF
  ...
  .filter(year($"IncidentDate")===lit("2018"))
  .groupBy($"semana")
  .agg(count("*").alias("n_llamadas"))
  .orderBy(desc("n_llamadas"))
  .limit(1)
)
